...
$deletes = Array.new
$adds = Array.new
outputFile = File.open("../output/Group_results_#{Time.now.strftime("%Y%m%d")}.txt", 'w')
...
def parse(input_file, output_array)
	...
	doc.search('Group').map do |group|
		output_array << [group['org_pac_id'], group.at('org_legal_name').text, group.at('GPRO_status').text, group.at('ERX_status').text, group.at('par_status').text, group.at('Location')['adrs_id']]
	end
end
...
until valid
	...
	1 medical-prod-predeploy.my.com
	2 qa.medical.gov
	3 staging.medical.gov
	4 www.medical.gov"
	
	environment = gets.strip!
	case environment
		when "1"
			env = "medical-prod-predeploy.my.com/physicianservices/Provider.svc/GetList/GETGRPGENINFO"
			valid = true
		when "2"
			env = "qa.medical.gov/physicianservices/Provider.svc/GetList/GETGRPGENINFO"
			valid = true
		when "3"
			env = "staging.medical.gov/physicianservices/Provider.svc/GetList/GETGRPGENINFO"
			valid = true
		when "4"
			env = "www.medical.gov/physicianservices/Provider.svc/GetList/GETGRPGENINFO"
			valid = true
		when "5"
			env = "medical-iterationb1.my.com/physicianservices/Provider.svc/GetList/GETGRPGENINFO"
			env2 = "medical-iterationb1.my.com/physicianservices/Provider.svc/GetList/GETGPRFLOCINFO"
			valid = true
		else
		   puts "Invalid input\nPlease try again\n\n"
	end
...
$adds[0...sample*$adds.count].each do |item|
	...
	name = json["GetListResult"][0]["Name"]
...
$deletes[0...sample*$adds.count].each do |item|
	group_id = item[0]
	begin
		response = Curl::Easy.perform("http://#{env}/sltdid%7c#{group_id}") do |curl| 
			curl.headers["cookie"] = "ASPSessionKey="
