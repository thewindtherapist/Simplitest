...
failed = Array.new
miss = Array.new
outputFile = File.open("../output/Provider_results#{Time.now.strftime("%Y%m%d")}.txt", 'w')
...
until valid
	...
	case environment
		when "1"
			env = "medical-prod-predeploy.my.com/physicianservices/Provider.svc/GetList/GETPRFLOCINFO"
			valid = true
		when "2"
			env = "qa.medical.gov/physicianservices/Provider.svc/GetList/GETPRFLOCINFO"
			valid = true
		when "3"
			env = "staging.medical.gov/physicianservices/Provider.svc/GetList/GETPRFLOCINFO"
			valid = true
		when "4"
			env = "www.medical.gov/physicianservices/Provider.svc/GetList/GETPRFLOCINFO"
			valid = true
		when "5"
			env ="medical-iterationb1.my.com/physicianservices/Provider.svc/GetList/GETPRFGENINFO"
			valid = true
		else
		   puts "Invalid input\nPlease try again\n\n"
	end
...
$adds[0...sample*$adds.count].each do |item|
	...
	if missing.empty?
		...
		outputFile.write("FAILED + Added + #{provider_id} #{miss}\n")
		print "\n"
	end#end if
end#end $records
